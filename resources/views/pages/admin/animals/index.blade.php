                ...
                <div class="card-body">
                    ...
                    <table class="datatable table">
                        ...
                        @foreach($data['animals'] as $index => $animal)
                            ...
                            <td> 
                            <button class="btn btn-warning  m-1" data-toggle="modal" data-target="#edit-animal-{{$animal->id}}">{{ __('lang.edit') }}</button>
                            <button type="button" class="btn btn-danger m-1" data-toggle="modal" data-target="#delete-animal-{{ $animal->id }}">{{ __('lang.delete') }}</button>
                            </td>
                            </tr>
                        @endforeach
                        </tbody>
...
<div class="modal fade" id="delete-animal-{{ $animal->id }}" aria-hidden="true" style="display: none;">
    ...
    <div class="modal-content border-danger">
        <form action="{{ route('admins.animals.destroy', $animal->id) }}" method="POST" enctype="multipart/form-data" >
            {{ method_field('DELETE') }}
            <input name="_token" type="hidden" value="{{ csrf_token() }}"/>
